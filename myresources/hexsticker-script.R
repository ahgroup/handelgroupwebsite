...
mysticker <- ggplot() + geom_hexagon(size = 2, fill = "white", color = "blue") +
                      ...
                      geom_pkgname(package="Handel \n Group", x = 1, y = 1, color = "black", family = "sans", size=20) +
                      geom_url(url = 'handelgroup.uga.edu',family = 'sans', size = 6) +
                      theme_sticker()
plot(mysticker)
ggsave(filename=here('assets/images',"icon.png"), plot = mysticker)
ggsave(filename=here('assets/images',"icon-32.png"), plot = mysticker, width = 43.9, height = 50.8, bg = 'transparent',  units = "mm", family = "sans")
